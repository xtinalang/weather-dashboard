...
class TestWeatherPage:
    ...
    async def test_weather_page_loads_with_valid_coordinates(self, page: Page):
        """Test that weather page loads with valid coordinates."""
        # Using London coordinates as an example
        await page.goto(f"{HOST}/weather/{LONDON_COORDINATES}")

        # Wait for page to load
        await page.wait_for_load_state("networkidle")

        # Check that we're on a weather page
        await expect(page.locator("h2")).to_contain_text(HEADING_WEATHER_FOR_PREFIX)

        # Check that basic weather info is displayed
        await expect(page.locator(".weather-display")).to_be_visible()
        await expect(page.locator(CSS_CLASS_TEMPERATURE)).to_be_visible()

    async def test_weather_page_title_format(self, page: Page):
        """Test that the page title follows the correct format."""
        await page.goto(f"{HOST}/weather/51.5074/-0.1278")
        await page.wait_for_load_state("networkidle")
    ...
    async def test_weather_location_info_display(self, page: Page):
        ...
        await expect(location_heading).to_contain_text("Weather for")
    ...
    async def test_weather_condition_display(self, page: Page):
        ...
        await expect(weather_card).to_be_visible()

        # Check condition text
        condition = page.locator(".condition")
        await expect(condition).to_be_visible()

        # Check temperature display
        temperature = page.locator(".temperature")
        await expect(temperature).to_be_visible()
    ...
    async def test_weather_icon_display(self, page: Page):
        ...
        if await weather_icon.is_visible():
            await expect(weather_icon).to_have_attribute("src")
            await expect(weather_icon).to_have_attribute("alt")

    async def test_feels_like_temperature_display(self, page: Page):
        ...
        await expect(feels_like).to_be_visible()

    async def test_weather_details_section(self, page: Page):
        ...
        await expect(page.locator(f"text=/{LABEL_HUMIDITY}/")).to_be_visible()
        await expect(page.locator(f"text=/{LABEL_WIND}/")).to_be_visible()
        await expect(page.locator(f"text=/{LABEL_PRESSURE}/")).to_be_visible()
        await expect(page.locator(f"text=/{LABEL_PRECIPITATION}/")).to_be_visible()
        await expect(page.locator(f"text=/{LABEL_UV_INDEX}/")).to_be_visible()

    async def test_weather_navigation_links_present(self, page: Page):
        ...
        await expect(page.locator(f"a:has-text('{LINK_BACK_TO_HOME}')")).to_be_visible()
        await expect(
            page.locator(f"a:has-text('{LINK_VIEW_FORECAST}')")
        ).to_be_visible()
        await expect(page.locator("a")).to_contain_text("Switch to °")

    async def test_back_to_home_link(self, page: Page):
    ...
    async def test_view_forecast_link(self, page: Page):
    ...
    async def test_weather_unit_switching_celsius_to_fahrenheit(self, page: Page):
        """Test switching from Celsius to Fahrenheit."""
        await page.goto(f"{HOST}/weather/{LONDON_COORDINATES}?unit=C")
        await page.wait_for_load_state("networkidle")
    ...
    async def test_weather_unit_switching_fahrenheit_to_celsius(self, page: Page):
        """Test switching from Fahrenheit to Celsius."""
        await page.goto(f"{HOST}/weather/{LONDON_COORDINATES}?unit=F")
        await page.wait_for_load_state("networkidle")
    ...
    async def test_weather_favorites_button_presence(self, page: Page):
    ...
    async def test_last_updated_timestamp(self, page: Page):
        ...
        await expect(last_updated).to_be_visible()

    async def test_weather_data_format_validation(self, page: Page):
        """Test that weather data is displayed in proper format."""
        await page.goto(f"{HOST}/weather/{LONDON_COORDINATES}")
        await page.wait_for_load_state("networkidle")

        # Check humidity format (should be percentage)
        humidity_text = await page.locator(f"text=/{LABEL_HUMIDITY}/").text_content()
        assert HUMIDITY_UNIT in humidity_text

        # Check pressure format (should have mb units)
        pressure_text = await page.locator(f"text=/{LABEL_PRESSURE}/").text_content()
        assert PRESSURE_UNIT in pressure_text

        # Check precipitation format (should have mm units)
        precip_text = await page.locator(f"text=/{LABEL_PRECIPITATION}/").text_content()
        assert PRECIPITATION_UNIT in precip_text

    async def test_wind_information_display(self, page: Page):
        """Test that wind information is properly displayed with direction."""
        await page.goto(f"{HOST}/weather/{LONDON_COORDINATES}")
        await page.wait_for_load_state("networkidle")

        # Check wind information
        wind_text = await page.locator(f"text=/{LABEL_WIND}/").text_content()
    ...
    async def test_weather_error_handling_invalid_coordinates(self, page: Page):
        """Test error handling for invalid coordinates."""
        # Try to access weather page with invalid coordinates
        await page.goto(f"{HOST}/weather/999/999")
        await page.wait_for_load_state("networkidle")
    ...
    async def test_weather_responsive_design_mobile(self, page: Page):
        ...
        await expect(page.locator(".temperature")).to_be_visible()
        await expect(page.locator(".weather-details")).to_be_visible()
        await expect(page.locator(".nav")).to_be_visible()

    async def test_weather_flash_messages_display(self, page: Page):
